...
class DemoCockpit:
    def __init__(
        ...
        eGap = 0
        engine_pos[0] = (
            horizon_pos[0] + horizon_size[0] + gap,
            horizon_pos[1],
        )  # Arrangement
        engine_pos[3] = (
            (engine_pos[0])[0] + engine_size[0] + eGap,
            (engine_pos[0])[1],
        )  # [0] [3]
        engine_pos[1] = (
            (engine_pos[0])[0],
            (engine_pos[0])[1] + engine_size[1] + eGap,
        )  #   X
        ...
        map_pos = (
            steeringWheel_pos[0] + steeringWheel_size[0] + gap,
            steeringWheel_pos[1],
        )
        ...
        self.engine = [0] * 4
        self.engine[0] = COMP.Percentage(
            self.screen, pos=engine_pos[0], size=engine_size
        )
        self.engine[1] = SPFB.SetPointVsFeedbackFill(
            self.screen, pos=engine_pos[1], size=engine_size
        )
        self.engine[2] = COMP.PercentageFill(
            self.screen, pos=engine_pos[2], size=engine_size
        )
        self.engine[3] = COMP.PercentageFill(
            self.screen, pos=engine_pos[3], size=engine_size
        )
        ...
        self.Ibat = BAT.Battery(
            self.screen, pos=txBatt_pos, size=battLevel_size, inputMin=0, inputMax=6
        )
        self.alt = ALT.AltMeter(
            self.screen,
            pos=alt_pos,
            size=alt_size,
        ...
        self.steeringWheel = SW.SteeringWheel(
            self.screen,
            pos=steeringWheel_pos,
            size=steeringWheel_size,
    ...
    def update(self, newData):
        ...
        self.engine[1].update(
            val=newData["RX_eng"] + random.randrange(-5, 5), valB=newData["RX_eng"]
        )
        ...
        self.head.update(
            newData["RX_head"] + random.randrange(-5, 5),
            newData["RX_head"] + random.randrange(-5, 5),
        )
        ...
        self.map.update(
            x=newData["RX_posX"], y=newData["RX_posY"], deg=newData["RX_head"]
        )
        self.world.update(
            x=newData["RX_worldX"],
            y=newData["RX_worldY"],
