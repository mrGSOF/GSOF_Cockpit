...
class DemoCockpit():
    ...
    GRAY  = (10,10,10 )
    BLUE  = (0,0,255 )
    def __init__(self, screen, pos=(0,0), scale=1.0, colorBG=BLACK, gap=0, folder='./'):
        self.screen = screen
        ...
        self.battTitle = Text( screen=self.screen,
                               ...
                               color=colorBG, textColor=self.WHITE,
                               name="<--[V] Batt [A]-->" )

        self.testBtn = Button_Rect( screen=self.screen,
                                    pos=testBtn_pos, size=None,
                                    color=COLOR.RED, textColor=self.WHITE,
                                    name="testButton" )
        ...
        axis  = Object(filename="%s/objects/axis.json"%folder, color=self.GRAY, name="Axis").setCenter(pos=(0, 0, 0), scale=10)
        plane = Object(filename="%s/objects/F16.stl"%folder, color=self.BLUE, name="F16").setCenter(method="arithCenter")
        world = Assembly(objects=(axis, plane))
