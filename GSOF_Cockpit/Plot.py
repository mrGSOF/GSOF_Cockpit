    ...
    def draw(self):
        ...
        if self.scanPos > self.right:
            drawLine(
                self._dial,
                overlayColor,
                (self.scanPos - 1, self.top),
                (self.scanPos - 1, self.bottom),
                4,
            )
            self.scanPos = self.left
        ...
        drawLine(
            self._dial, 0xFFFFFF, (scanPos, self.top), (scanPos, self.bottom), 4
        )  #< Draw the cursor as vertical line
        drawLine(
            self._dial,
            overlayColor,
            (scanPos - 1, self.top),
            (scanPos - 1, self.bottom),
            4,
        )  #< Clear the points of the previous plot
        ...
        for plot, middle, color in zip(self.plots, self.middles, self.colors):
            ...
            if self.style == "dot":
                drawLine(
                    self._dial,
                    color,
                    (scanPos - 1, middle - val * 0.98),
                    (scanPos - 1, middle - val),
                    4,
                )  #< Draw dot plot
            else:
                drawLine(
            ...
            drawLine(
                self._dial, GRAY, (self.left, middle), (self.right, middle)
            )  #< Draw center line
