...
class GMeter_Analog(SingleIndicator):
    ...
    def __init__(
        ...
        if minMaxHandImage == None:
            minMaxImage = imageLoad(
                os.path.join(path, "../skin/Alt_Meter200_S_Needle.png")
            )
        self._minHand = Hand(
            initVal=0.0,
            offset=0.0,
        ...
        self.resetG()
        self.rstBtn = Button_Round(
            screen=screen,
            pos=(pos[0] + 10, pos[1] + 118),
            size=(25, 25),
    ...
    def _drawMinMaxMarks(self, draw=True):
        if self._minMaxChg == True:
            self.__minHand = rotate(
                self._minHand.skin, int(self._minHand.angle_deg)
            )  #< Rotate the minimum mark
            self.__maxHand = rotate(
                self._maxHand.skin, int(self._maxHand.angle_deg)
            )  #< Rotate the maximum mark
