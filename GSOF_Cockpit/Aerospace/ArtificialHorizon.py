...
class ArtificialHorizon(Gauge):
    ...
    def __init__(
        ...
        pitchKp=0.5,
    ):  #< Filter coefficient (0-no filter)
        """Artificial horizon gauge. Default size of 300px can be overridden using w,h"""
        path = os.path.dirname(__file__)
        if bodyImage == None:
            bodyImage = imageLoad(
                os.path.join(path, "../resources/Horizon_Background.png")
            )

        if ballImage == None:
            ballImage = imageLoad(
                os.path.join(path, "../resources/Horizon_GroundSky.png")
            )

        if birdImage == None:
            birdImage = imageLoad(os.path.join(path, "../resources/Maquette_Avion.png"))

        super().__init__(screen, bodyImage=bodyImage, pos=pos, size=size)
    ...
    def draw(self, draw=True):
        ...
        if self._icon != None:
            ...
            )  #< Overlay the bird icon on gauge
