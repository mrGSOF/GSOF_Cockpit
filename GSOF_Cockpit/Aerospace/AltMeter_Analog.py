...
class AltMeter(DualIndicator):
    ...
    def __init__(
        ...
        super().__init__(
            screen=screen,
            bodyImage=bodyImage,
            handAImage=handAImage,
            handBImage=handBImage,
            pos=pos,
            size=size,
            # initVal     = initVal,
            ...
            inputAtoDeg=-360.0
            / 1000,  #< Input value to degrees factor applied after offset
        ...
        self.altitude = 0.0
        self.digitalDisp = Text(
            screen=screen,
            pos=(pos[0] + int(size[0] * 0.15), pos[1] + int(size[1] * 0.44)),
            size=(int(0.27 * size[0]), int(0.14 * size[1])),
            color=None,
