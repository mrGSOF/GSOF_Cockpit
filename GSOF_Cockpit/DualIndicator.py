...
class DualIndicator(SingleIndicator):
    ...
    def __init__(
        ...
        if handBImage == None:
            handBImage = imageLoad(
                os.path.join(path, "resources/SmallNeedleAltimeter.png")
            )
        self._handB = Hand(
            initVal=initVal,
            offset=inputOffset,
        ...
        if bodyImage == None:
            bodyImage = imageLoad(
                os.path.join(path, "resources/Altimeter_Background.png")
            )
        if handAImage == None:
            handAImage = imageLoad(
                os.path.join(path, "resources/LongNeedleAltimeter.png")
            )
        super().__init__(
            screen=screen,
            bodyImage=bodyImage,
            handImage=handAImage,
            pos=pos,
            size=size,
            initVal=initVal,
            inputGain=inputGain,
    ...
    def draw(self, draw=True):
        ...
        handB = rotate(
            self._handB.skin, int(self._handB.angle_deg)
        )  #< Rotate the hand
